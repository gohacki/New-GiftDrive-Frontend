...
const { EMAIL_HOST, EMAIL_PORT, EMAIL_USER, EMAIL_PASS, EMAIL_FROM_ADDRESS, EMAIL_SECURE } = process.env;

if (!EMAIL_HOST || !EMAIL_PORT || !EMAIL_USER || !EMAIL_PASS || !EMAIL_FROM_ADDRESS) {
    console.error("Email service is not configured. Missing one or more required environment variables (EMAIL_HOST, EMAIL_PORT, EMAIL_USER, EMAIL_PASS, EMAIL_FROM_ADDRESS).");
}
...
export async function sendEmail({ to, subject, text, html }) {
    ...
    const mailOptions = {
        from: `"GiftDrive" <${EMAIL_FROM_ADDRESS}>`,
        to,
        subject,
        text,
        html,

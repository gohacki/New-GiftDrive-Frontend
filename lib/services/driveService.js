...
export async function getDriveAggregates(driveId) {
    ...
    // Total Needed: Excludes items marked as hidden_from_public
    ...
    const [[childItemsNeededRow]] = await pool.query(
        `SELECT COALESCE(SUM(ci.quantity), 0) AS total_child_needed
         ...
         WHERE uc.drive_id = ? AND ci.is_active = 1 AND ci.is_hidden_from_public = FALSE`, // Renamed column
        [numericDriveId]
    );
    ...
    // Total Purchased: Remains based on actual orders, regardless of current hidden status of the original need item
...
export async function getDriveTopDonors(driveId) {
    // This function remains unchanged as donor activity is historical
    // and not affected by current item visibility.
    const numericDriveId = parseInt(driveId, 10);
    if (isNaN(numericDriveId)) {
        throw new Error("Invalid Drive ID format.");
    }
...
export async function getDriveRecentDonations(driveId) {
    // This function also remains unchanged for the same reasons as topDonors.
    const numericDriveId = parseInt(driveId, 10);
    if (isNaN(numericDriveId)) {
        throw new Error("Invalid Drive ID format.");
    }

...
// Helper to format time ago (from your pages/api/drives/[driveId]/recent-donations.js)
...
/**
 * Fetches core drive details along with organization info and basic children info.
 * (Logic from pages/api/drives/[driveId].js GET handler)
 */
...
/**
 * Fetches aggregate data (totalNeeded, totalPurchased, donorsCount) for a drive.
 * (Logic from pages/api/drives/[driveId]/aggregate.js)
 */
...
/**
 * Fetches items specifically associated with a drive (not child items).
 * (Logic from pages/api/drives/[driveId]/items/index.js GET handler)
 */
export async function getDriveSpecificItems(driveId) {
    ...
    const [rows] = await pool.query(`
        SELECT
          ...
          -- Ensure base_marketplace_store_domain is selected if it exists in your 'items' table
          i.store_domain AS base_marketplace_store_domain, 
    ...
    return rows.map(r => ({
        ...
        base_marketplace_store_domain: r.base_marketplace_store_domain, // Add this
        is_rye_linked: Boolean(r.base_is_rye_linked),
    }));
}

/**
 * Fetches top donors for a specific drive.
 * (Logic from pages/api/drives/[driveId]/top-donors.js)
 */
export async function getDriveTopDonors(driveId) {
    ...
    return topDonors.map(donor => ({
        ...
        badge: '' // Placeholder if not used
    }));
}

/**
 * Fetches recent donations for a specific drive.
 * (Logic from pages/api/drives/[driveId]/recent-donations.js)
 */
